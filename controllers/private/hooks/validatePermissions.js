module.exports = async function (req, reply) {
    ...
    const { id_usuario } = await this.$info(req)
    const error = { message: 'No tienes permisos suficientes', statusCode: 'No Action', code: 'noAction' }
    if (!req.headers.ruta) {
        const menus = await menuUsuarioService.getMenus(id_usuario)
        error.menus = menus
        error.permission = null
        reply.code(401)
        reply.send(error)
    }
    let { id_permiso, metodo } = (await menuUsuarioService.validateMenuUsuario({ id_usuario, ruta: req.headers.ruta })) || {}
